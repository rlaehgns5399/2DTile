...
namespace XDOErrorDetectorUI
{
    public class GLTF
    {
        ...
        private void create(List<int> makeList)
        {
            ...
            foreach(var availableIndex in makeList)
            {
                // Skeleton
                this.container = JObject.Parse(@"{
             }");
                ...
                for (int i = 0; i < xdo.faceNum; i++)
                {
                    ...
                    // define images
                    // JArray images = new JArray();
                    var imgName = xdo.mesh[i].imageName.Replace(".jpg", "_" + availableIndex + ".jpg");
                    ...
                    File.Copy(Path.Combine(new FileInfo(xdo.url).Directory.FullName, imgName), Path.Combine(savePath, imgName.ToLower()), true);
                ...
                StreamWriter sw = new StreamWriter(Path.Combine(savePath, getFileNameWithoutExtension) + "_" + availableIndex + ".gltf");
                if (debug) sw.Write(container.ToString(Newtonsoft.Json.Formatting.None));
                if (!debug) sw.Write(container.ToString());
                sw.Close();
            }
